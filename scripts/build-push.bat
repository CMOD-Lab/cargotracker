...
for /f %%i in ('powershell -NoProfile -Command "$n='%PROJECT_NAME%'.ToLower(); $n=[regex]::Replace($n,'[^a-z0-9]+','-'); $n=$n.Trim('-'); if([string]::IsNullOrWhiteSpace($n)){$n='cargo-tracker'}; Write-Output $n"') do set "IMAGE_NAME=%%i"
...
echo 1^) Azure ACR
echo 2^) Docker Hub
set /p REGISTRY_CHOICE="Enter selection [1-2]: "
...
if "!REGISTRY_CHOICE!"=="1" (
  ...
  set /p DOCKER_USERNAME="Enter Docker Hub username or organization: "
  if "!DOCKER_USERNAME!"=="" (
